...
function getLinkVar(placeHolder, obj, attr, link) {
    if (attr === 'protocol') {
        attr = 'secure';
    }
    if (placeHolder === 'ip') {
...
function processOneWelcome(welcomeScreen, isPro, adapterObj, foundInstanceIDs, instances, hosts, hostname, webNamespace, list) {
    ...
    if (ws.localLinks && typeof ws.localLinks === 'string') {
        if (adapterObj.common.localLinks && typeof adapterObj.common.localLinks[ws.localLinks] === 'object') {
            localLink = adapterObj.common.localLinks[ws.localLinks].link;
        }
        else {
 ...
 */
function addImplicitHeader(_req, res, next) {
    const response = res;
...
class WebAdapter extends adapter_core_1.Adapter {
    ...
    store = null;
    secret = 'Zgfr56gFe87jJOM'; // Will be generated by first start
    socketUrl = '';
     ...
     */
    static REMOTE_COMMAND = /^(cloud\.\d+)\.remote\.command$/;
    /**
    ...
    onObjectChange(id, obj) {
        if (this.ownGroups && id.startsWith('system.group.')) {
            if (obj) {
                this.ownGroups[id] = obj;
            }
            else {
                delete this.ownGroups[id];
            }
        }
        if (this.ownUsers && id.startsWith('system.user.')) {
            if (obj) {
                this.ownUsers[id] = obj;
            }
            else {
    ...
    onMessage(msg) {
        if (msg?.command === 'getUserByCookie') {
            ...
            if (cookie) {
                try {
                    cookie = cookie_signature_1.default.unsign(decodeURIComponent(cookie).slice(2), this.secret);
                }
                catch (e) {
                ...
                try {
                    if (this.extensions?.[instance]?.obj?.unload) {
                        ...
                        if (promise instanceof Promise) {
                            promises.push(promise.catch(e => this?.log?.error(`Cannot unload web extension "${instance}": ${e}`)));
                        }
                    }
                }
                catch (e) {
    ...
    async onReady() {
        ...
        if (systemConfig) {
            if (!systemConfig?.native?.secret) {
                ...
                const buf = await new Promise(resolve => (0, node_crypto_1.randomBytes)(24, (_err, buf) => resolve(buf)));
    ...
    async getExtensionsAndSettings() {
    ...
    async getListOfAllAdapters(remoteIp) {
        ...
        for (let a = 0; a < adapters.rows.length; a++) {
            ...
            if (instances?.rows) {
                ...
                for (let i = 0; i < instances.rows.length; i++) {
                    ...
                    const id = instances.rows[i].id.replace(/\.\d+$/, '');
                    if (id === obj._id &&
                ...
                if (obj.common.localLinks && typeof obj.common.localLinks === 'object') {
                    for (const link in obj.common.localLinks) {
                        if (typeof obj.common.localLinks[link] === 'object') {
                            let name = obj.common.localLinks[link].name || obj.common.name;
        ...
        for (const listItem of uniqueList) {
            if (typeof listItem.localLink === 'string') {
                listItem.localLink = this.resolveLink(listItem.localLink, listItem.id ? mapInstances[listItem.id] : undefined, mapInstances);
            }
            if (!listItem.name) {
     ...
     */
    resolveLink(link, instanceObj, instancesMap) {
            ...
            for (const placeHolder of vars) {
                ...
                if (adapterName) {
                    ...
                    instances[adapterName] ||= Object.values(instancesMap)
                        .map(inst => inst._id.startsWith(`system.adapter.${adapterName}.`)
                    ...
                    if (!instances[adapterName].length) {
                        ...
                        for (const instance of instances[adapterName]) {
                            links[instance] = {
                                instance,
                                link: getLinkVar(shortPlaceHolder, instancesMap[`system.adapter.${instance}`], variableName, links[instance]?.link || link),
                            };
                        }
                    }
                }
    ...
    initAuth() {
        ...
        passport_1.default.deserializeUser((serializedUser, done) => done(null, serializedUser));
        ...
        if (this.config.oauth && !this.config.auth) {
            ...
            if (!this.config.secure && !this.config.publicUrl?.startsWith('https://')) {
                this.log.warn('OAuth without HTTPS: remote clients refuse plain http:// and will not be able to connect. ' +
        ...
        this.webServer.app.use(passport_1.default.initialize());
        this.webServer.app.use(passport_1.default.session());
        this.webServer.app.use((0, connect_flash_1.default)());
    }
    /**
                ...
                if (file && (0, node_fs_1.existsSync)(file)) {
                    ...
                    try {
                        ...
                        if ((0, node_fs_1.existsSync)(`${fileDir}socket.io.min.js`)) {
                            ...
                            this.socketIoFile = (0, node_fs_1.readFileSync)(`${fileDir}socket.io.js`);
                        }
                    }
                    catch {
                        try {
                            // if nothing works, read stored in a web file
                            this.socketIoFile = (0, node_fs_1.readFileSync)(`${__dirname}/${wwwDir}/lib/js/socket.io.js`);
                        }
                        catch (e) {
    ...
    async processReadFolders(req, res) {
     ...
     */
    async applyRemoteCommand(stateName, value, user) {
        ...
        try {
            if (!this.checkedRemoteDevices.has(deviceId)) {
                ...
                if (!(await this.getForeignObjectAsync(aliveId, { user }))) {
                    await this.setForeignObjectAsync(aliveId, {
                        type: 'state',
                        common: {
            ...
            if (command.name === 'alive') {
                ...
                await this.setForeignStateAsync(aliveId, {
                    val: command.value === true ||
                        command.value === 'true' ||
                        command.value === 1 ||
                        command.value === '1',
                    ack: true,
            ...
            if (!obj) {
                ...
                if (command.name === 'currentLocation') {
                    ...
                    if (!(await this.getForeignObjectAsync(positionId, { user }))) {
                        await this.setForeignObjectAsync(positionId, {
                            type: 'state',
                            common: {
                                name: 'Position',
                                type: 'string',
            ...
            if (command.name === 'currentLocation') {
                try {
                    const location = JSON.parse(command.value);
                    await this.setForeignStateAsync(`${deviceId}.position`, { val: `${location?.coords?.longitude};${location?.coords?.latitude}`, ack: true }, { user });
                }
                catch {
     ...
     * @param user the user the request is running as
     */
    async createVisState(stateId, common, user) {
    ...
    static remoteStateCommon(name, value) {
        const common = {
            name,
            type: typeof value,
            role: 'state',
            ...
            if (!this.config.disableStates) {
                ...
                this.webServer.app.post('/state/:stateId', async (req, res) => {
                    try {
                        ...
                        const user = req.user ? `system.user.${req.user}` : this.config.defaultUser;
                        // Read post
                        ...
                        if (!obj && WebAdapter.REMOTE_COMMAND.test(stateName)) {
                            ...
                            try {
                                await this.setForeignObjectAsync(stateName, {
                                    type: 'state',
                                    common: {
        ...
        if (this.ownSocket) {
            ...
            try {
                if (this.config.usePureWebSockets) {
                    this.webServer.io = new ws_server_library_1.IOSocketClass(this.webServer.server, socketSettings, this, this.store, this.checkUser);
                }
                else {
                    this.webServer.io = new socketio_server_1.IOSocketClass(this.webServer.server, socketSettings, this, this.store, this.checkUser);
                }
            }
            catch (err) {
        ...
        if (!this.config.disableExtensions) {
            ...
            Object.keys(this.extensions).forEach(instance => {
                try {
                    ...
                    parts.shift();
                    let extAPI;
                    if ((0, node_fs_1.existsSync)(`./${parts.join('/')}`)) {
                    ...
                    if (this.extensions[instance].obj?.waitForReady &&
                        ...
                        extensionPromises.push(new Promise(resolve => {
                            let timeout = this.setTimeout(() => {
                                if (timeout) {
                                    timeout = undefined;
                                    this.log.error(`Extension "${instance}" (${this.extensions[instance].path}) is not responding (waitForReady)`);
                                    resolve();
                                }
...
if (require.main !== module) {
    ...
    module.exports = (options) => new WebAdapter(options);
}
else {
